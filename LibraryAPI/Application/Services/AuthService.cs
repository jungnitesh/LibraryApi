        ...
        public async Task<RegisterResponseDto> RegisterAsync(RegisterRequestDto request)
        {
            ...
            try
            {
                ...
                if (!result.Succeeded)
                {
                    ...
                    throw new AuthException($"User creation failed: {message}",null);
                }

                var roleResult = await userManager.AddToRoleAsync(user, request.Role);
                if (!roleResult.Succeeded)
                {
                    throw new AuthException("User role assignment failed.",null);
                }
            ...
            catch(Exception Ex)
            {
                ...
                throw new DatabaseException("Failed",Ex.InnerException);
            }
        }
